...
Route *getRoutes(int *routeCount) {
  ...
  if (routeList == NULL) {
    ...
    freeData(responseList,
             *routeCount); // Liberamos responseList en caso de fallo
    return NULL;
  }
  ...
  for (int i = 0; i < *routeCount; i++) {
    if (responseList[i] != NULL) {
      if (responseList[i][0] != NULL)
        strncpy(routeList[i].id, responseList[i][0],
                sizeof(routeList[i].id) - 1);
      else
        routeList[i].id[0] = '\0';

      if (responseList[i][1] != NULL)
        strncpy(routeList[i].name, responseList[i][1],
                sizeof(routeList[i].name) - 1);
      else
        routeList[i].name[0] = '\0';

      if (responseList[i][2] != NULL)
        strncpy(routeList[i].link, responseList[i][2],
                sizeof(routeList[i].link) - 1);
      else
