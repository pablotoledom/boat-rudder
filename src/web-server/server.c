    ...
    for (; *src && dst < dst_end; src++) {
        if (isalnum((unsigned char)*src) || strchr("-_.~/", *src)) {
            *dst++ = *src;
            ...
            if (dst + 3 >= dst_end) break; // There is not enough space
                ...
                while ((entry = readdir(dir)) != NULL) {
                    ...
                    if (strcmp(entry->d_name, "..") == 0) {
                        ...
                        if (strcmp(decoded_url, "") == 0 || strcmp(decoded_url, "/") == 0) {
                            // We are already in the root directory, do not show link to parent
                            continue;
                        } else {
                            ...
                            url_encode(encoded_item_path, parent_url, sizeof(encoded_item_path));
                        }
                    } else {
                    ...
                    len += snprintf(response + len, sizeof(response) - len,
                                    "<li><a href=\"%s\">%s</a></li>",
                                    encoded_item_path, display_name);
                }
                closedir(dir);
                len += snprintf(response + len, sizeof(response) - len, "</ul></body></html>");
