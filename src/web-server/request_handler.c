...
void handle_request(int client_socket, const char *root_directory) {
    ...
    if (S_ISDIR(path_stat.st_mode)) {
        ...
        if (snprintf(index_path, sizeof(index_path), "%s/index.html", safe_path) >= (int)sizeof(index_path)) {
            // Path too long
        ...
        if (stat(index_path, &path_stat) == 0) {
            ...
            if (snprintf(index_path, sizeof(index_path), "%s/index.htm", safe_path) >= (int)sizeof(index_path)) {
                // Path too long
                fprintf(stderr, "Path too long when building index_path\n");
                close(client_socket);
                return;
            ...
            if (stat(index_path, &path_stat) == 0) {
                ...
                while ((entry = readdir(dir)) != NULL) {
                    ...
                    if (snprintf(entry_full_path, sizeof(entry_full_path), "%s/%s", safe_path, entry->d_name) >= (int)sizeof(entry_full_path)) {
